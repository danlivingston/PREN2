...
def draw_cube(pos, color, z_pos):
    ax.bar3d(pos[0], pos[1], z_pos, cube_size, cube_size, cube_size, color=color, shade=True)
...
def update(frame):
    ...
    for i in range(len(cube_positions)):
        if frame >= i * 10:
            z_pos = max(cube_positions[i][2] - falling_speed * (frame - i * 10), cube_positions[i][2])
            draw_cube(cube_positions[i], colors[i], z_pos)

ani = FuncAnimation(fig, update, frames=10 * len(cube_positions), interval=50, repeat=False)
