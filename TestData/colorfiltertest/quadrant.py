...
def is_line_through_center(x1, y1, x2, y2, centerX, centerY, marginX, marginY):
    ...
    return (lowerBoundY <= y_at_centerX <= upperBoundY) and (lowerBoundX <= x_at_centerY <= upperBoundX)
...
while cap.isOpened():
    ...
    if lines is not None:
        for line in lines:
            x1, y1, x2, y2 = line[0]
            if is_line_through_center(x1, y1, x2, y2, centerX, centerY, marginX, marginY):
