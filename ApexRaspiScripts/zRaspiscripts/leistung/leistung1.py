...
def update_stats():
    ...
    net_label.config(
        text=f"Net Sent: {net_io.bytes_sent} bytes, Net Received: {net_io.bytes_recv} bytes"
    )
    uptime_label.config(
        text=f"System Uptime: {int(uptime_hours)}h {int(uptime_minutes)}m {int(uptime_seconds)}s"
    )
