...
# Globale Variable, um den Messvorgang zu steuern
running = False
...
def measure():
    ...
    while running:
        ...
        energy = read_energy()#*3600
        
        print(f"Spannung: {voltage} V, Strom: {current} A, Leistung: {power} W, Energie: {energy} Ws")
        time.sleep(5)
        root.after(1000, measure)  # Aktualisierungsrate auf 1 Sekunde gesetzt
...
root = tk.Tk()
root.title("Stromverbrauchsmessung")

start_button = tk.Button(root, text="Start", command=start_measurement)
start_button.pack(pady=20)

stop_button = tk.Button(root, text="Stop", command=stop_measurement)
