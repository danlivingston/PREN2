...
from cubepiler.bilderkennung.referenceQuarter import ReferenceQuarter
...
class CubeReconstruction:
    def __init__(self):
        self.colorModel = YOLO(f"{current_directory}/models/cube_segmentation_2.pt")
    ...
    def processDetections(self):
        for cdrf in self.cubeDetectionResultsFront:
            for i in range(len(cdrf.boxes.xyxyn)):
                ...
                self.cubes[i] = Cube(
                    ...
                    twoSided=self.getSidesNumber(cdrf.boxes.cls[i].item()),
                    view="front",
                    color=self.getColor(cdrf.boxes.cls[i].item()),
                    conf=cdrf.boxes.conf[i],
                )

        for rdrf in self.referenceDetectionResultsFront:
            for j in range(len(rdrf.boxes.xyxyn)):
                tempCoordinates = rdrf.boxes.xyxyn[j]
                self.references[j] = ReferenceQuarter(
                    j,
        ...
        for cdrb in self.cubeDetectionResultsBack:
            for i in range(len(cdrb.boxes.xyxyn)):
                ...
                self.cubesBack[i] = Cube(
                    i,
                    x1=tempCoordinates[0],
                    y1=tempCoordinates[1],
                    x2=tempCoordinates[2],
                    y2=tempCoordinates[3],
                    twoSided=self.getSidesNumber(cdrb.boxes.cls[i].item()),
                    view="back",
                    color=self.getColor(cdrb.boxes.cls[i].item()),
                    conf=cdrb.boxes.conf[i],
                )

        for rdrb in self.referenceDetectionResultsBack:
            for j in range(len(rdrb.boxes.xyxyn)):
                tempCoordinates = rdrb.boxes.xyxyn[j]
                self.referencesBack[j] = ReferenceQuarter(
                    j,
                    x_coordinate=tempCoordinates[0],
                    y_coordinate=tempCoordinates[1],
                    width=tempCoordinates[2],
    ...
    async def run_detection(self):
        ...
        return self.cubes_to_json(self.absolute_cubes)
